        ...
        with open(entity_catalogue, "r") as fin:
            ...
            for line in lines:
                ...
                if "kb_idx" in entity:
                    ...
                    id2wikidata[str(local_idx)] = int(entity["idx"].split("curid=")[-1])
                else:
                    id2wikidata[str(local_idx)] = -1
                local_idx += 1
        json.dump(id2title, open("models/id2title.json", "w"),indent=4)
        json.dump(id2text, open("models/id2text.json", "w"),indent=4)
        json.dump(id2wikidata, open("models/id2wikidata.json", "w"),indent=4)
    else:
...
def get_predictions(
    ...
    for batch_num, batch_data in enumerate(dataloader):
        ...
        for b in range(len(batch_context)):
            ...
            # candidate entity number of per mention
            candidate_num = 5
            # sort of mention
            _, sort_idxs = torch.tensor(distances[:,0][top_mentions_mask]).sort(descending=True)
            # cands already sorted by score
            all_pred_entities = pred_entity_list[:,:candidate_num][top_mentions_mask]
            e_mention_bounds = entity_mention_bounds_idx[top_mentions_mask]
            chosen_distances = distances[:,:candidate_num][top_mentions_mask] # (mention, candidate_num)
            ...
            for idx, mb in enumerate(e_mention_bounds):
                ...
                chosen_distances_pruned.append(list(map(float,chosen_distances[idx])) if isinstance(chosen_distances[idx],np.ndarray) else float(chosen_distances[idx]))
                mention_masked_utterance[mb[0]:mb[1]] = 1

            input_context = input_context[1:-1]  # remove BOS and sep
            pred_triples = [(
                #str(all_pred_entities_pruned[j]),
                list(map(str,all_pred_entities_pruned[j].tolist())),
            ...
            if 'label_id' in sample:
                ...
                entity_results.update({
                    ...
                    # ],
                    "pred_tuples_string":[
                        [(id2title[str(cand)],tokenizer.decode(input_context[triple[1]:triple[2]])) for cand in triple[0]]
                        for triple in pred_triples
                    ],
                    "pred_triples": pred_triples,
                    "tokens": input_context,
